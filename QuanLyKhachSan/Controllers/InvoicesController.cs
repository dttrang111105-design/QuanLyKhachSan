...
public class InvoicesController : Controller
{
    ...
    {
        ViewData["BookingId"] = new SelectList(_context.Bookings
                .Where(b => !b.IsDeleted && b.Status == BookingStatus.CheckedOut), "Id", "BookingCode");
        return View();
    }
    [HttpPost]
        ...
        Invoice invoice)
    {
        ...
        var booking = await _context.Bookings
            .FirstOrDefaultAsync(b => b.Id == invoice.BookingId && !b.IsDeleted);
        if (booking == null)
        {
            return NotFound();
        }
        ...
        bool existedInvoice = await _context.Invoices
            .AnyAsync(i => i.BookingId == invoice.BookingId && !i.IsDeleted);
        if (existedInvoice)
        {
            ModelState.AddModelError(string.Empty, "Booking này đã có hóa đơn.");
            await LoadBookingSelectList(invoice.BookingId);
            return View(invoice);
        }
        decimal roomAmount = await _context.BookingDetails
            .Where(d => d.BookingId == invoice.BookingId && !d.IsDeleted)
        ...
        decimal discountAmount = subTotal * invoice.DiscountPercent / 100;
        decimal amountAfterDiscount = subTotal - discountAmount;
        decimal taxAmount = amountAfterDiscount * invoice.TaxPercent / 100;
        ...
        Invoice form)
    {
        ...
        {
            ModelState.AddModelError(nameof(Invoice.DiscountPercent),
                "Giảm giá phải nằm trong khoảng từ 0 đến 100%.");
        }
        if (form.TaxPercent < 0 ||
            form.TaxPercent > 100)
        {
            ModelState.AddModelError(nameof(Invoice.TaxPercent),
        ...
        decimal discountAmount = subTotal * form.DiscountPercent / 100;
        decimal amountAfterDiscount = subTotal - discountAmount;
        decimal taxAmount = amountAfterDiscount * form.TaxPercent / 100;
    ...
    public async Task<IActionResult> MyInvoices()
    {
        ...
        var invoices = await _context.Invoices
            ...
            .Include(i => i.Payments.Where(p => !p.IsDeleted))
            .Where(i => !i.IsDeleted && i.Booking != null &&
                !i.Booking.IsDeleted && i.Booking.CustomerId == customer.Id)
            .OrderByDescending(i => i.InvoiceDate)
            .ToListAsync();
        return View(invoices);
    }
    ...
    public async Task<IActionResult> MyDetails(int id)
    {
        ...
        var invoice = await GetInvoiceDetailsQuery()
            .FirstOrDefaultAsync(i => i.Id == id && !i.IsDeleted &&
                i.Booking != null && i.Booking.CustomerId == customer.Id);
        if (invoice == null)
        {
            return NotFound();
        }
    ...
    private async Task AddInvoiceDetails(Invoice invoice)
    {
        ...
        var serviceBookings = await _context.ServiceBookings
            .AsNoTracking()
            .Include(s => s.Service)
            .Where(s => s.BookingId == invoice.BookingId && !s.IsDeleted)
            .ToListAsync();
    ...
    private async Task<Customer?> GetCurrentCustomer()
    {
        ...
        return await _context.Customers
            ...
            .FirstOrDefaultAsync(c =>
                !c.IsDeleted && c.Account != null &&
                !c.Account.IsDeleted && c.Account.Username == username);
    }
